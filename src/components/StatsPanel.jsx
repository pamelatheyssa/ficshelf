export default function StatsPanel({ fanfics }) {
  ...
  const avgWords = ficsWithWords.length > 0 ? Math.round(totalWords / ficsWithWords.length) : 0;
  const favorites = fanfics.filter(f => f.favorite).length;

  const authorCount = {};
  fanfics.forEach(f => { if (f.author) authorCount[f.author] = (authorCount[f.author] || 0) + 1; });
  ...
  const statCards = [
    ...
    { label: 'Média por fic', value: avgWords > 0 ? Number(avgWords).toLocaleString('pt-BR') + ' pal.' : '—', icon: '📊', color: 'var(--lilac)' },
    ...
    <div className="stats-panel">
      ...
      <div className="stats-platforms">
        ...
        <div className="platform-bar-track">
          <div className="platform-bar-fill fill-ao3" style={{ width: fanfics.length > 0 ? `${(ao3Count / fanfics.length) * 100}%` : '0%' }} />
        </div>
        <div className="platform-bar-label">
        ...
        <div className="platform-bar-track">
          <div className="platform-bar-fill fill-wattpad" style={{ width: fanfics.length > 0 ? `${(wattpadCount / fanfics.length) * 100}%` : '0%' }} />
        </div>
      </div>
    </div>
  );
