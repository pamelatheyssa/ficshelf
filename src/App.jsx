...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState('want'); // 'want' | 'read'
  const [subTab, setSubTab] = useState('all'); // 'all' | 'complete' | 'incomplete'
  const [search, setSearch] = useState('');
  const [modal, setModal] = useState(null); // null | {type, fanfic?}
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Remover esta fanfic?')) {
  ...
  return (
    <div className="app">
      ...
      <main className="main">
        {/* Sub-tabs for want to read */}
        {activeTab === 'want' && (
          <div className="subtabs">
            <button className={`subtab-btn ${subTab === 'all' ? 'active' : ''}`}
              onClick={() => setSubTab('all')}>
              Todas ({counts.want})
            </button>
            <button className={`subtab-btn ${subTab === 'complete' ? 'active' : ''}`}
              onClick={() => setSubTab('complete')}>
              ✅ Completas ({counts.wantComplete})
            </button>
            <button className={`subtab-btn ${subTab === 'incomplete' ? 'active' : ''}`}
        ...
        <div className="toolbar">
          ...
          <button className="add-btn"
            onClick={() => setModal({ type: 'add', defaultStatus: activeTab })}>
            + Adicionar fanfic
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <h3 className="empty-title">
              {search ? 'Nenhuma fanfic encontrada' : activeTab === 'want' ? 'Sua lista está vazia' : 'Nenhuma fanfic lida ainda'}
            </h3>
            <p className="empty-text">
              {search ? 'Tente outro termo de busca.' : 'Clique em "+ Adicionar fanfic" para começar!'}
            </p>
          </div>
        ) : (
